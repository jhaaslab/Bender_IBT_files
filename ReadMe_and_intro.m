...
% annotation; there's lots more where this came from.
xlabel('ms')
ylabel('mV')
title('Stuff')
text(.2, 30, 'Hello!')

% The first set of f/i data is hte first 16 sweeps
set_one=D(1:16);  
figure(12); clf; 
subplot(121);hold on;
...
% Take & work with only the first 5 sweeps, which had current inputs from
% from -100 to 0, to get input resistance from a fit.
N=5; 
for i=1:N
    sweep=set_one(i).data;
...
% now work with the full set to get mean spiking frequency.
subplot(224);
for i=1:length(set_one)
